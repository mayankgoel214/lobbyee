...
- **Security:** DB password + service-role key already ROTATED via the 2026-06-28 Supabase project migration (fresh project = fresh creds; old `hmpdatpvevkhnaieuqeq` paused).
  - **Rate limiting: DONE & LIVE IN PROD** — Postgres-backed fixed-window limiter (`lib/rate-limit.ts`, migration `11_rate_limit`, RLS-on service-path table). Applied to signup/signin/magic-link/oauth/resend (by IP + email), password-change, sendTurn (`turn:<uid>`), AI depth-suggest (`suggest:<uid>`), and voice-session-token (`voice-token:<uid>`). Fails OPEN on DB error; `clientIp()` prefers non-spoofable `x-real-ip`; `cleanupRateLimit()` swept by the eval-drain cron. Verified 2026-07-20 (table present, RLS on, active buckets in prod). Swap to Redis token-bucket only if scale demands.
  - **Sentry: WIRED (server + edge), env-gated on `SENTRY_DSN`** — `@sentry/nextjs` installed, `instrumentation.ts` + `sentry.server.config.ts` + `sentry.edge.config.ts` + `sentry.scrub.ts` (PII-scrub, `sendDefaultPii:false`, `tracesSampleRate:0`). Webhook + eval routes call it. Client-side capture DELIBERATELY omitted (lean bundle). **To activate: set `SENTRY_DSN` in Vercel** — no code change needed. Optional later: `withSentryConfig` in next.config for readable (source-mapped) stack traces.
