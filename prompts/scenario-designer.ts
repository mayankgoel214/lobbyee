...
export const SCENARIO_DESIGNER_VERSION = "scenario-designer@v2";

export function renderScenarioDesignerPrompt(input: {
  title: string;
  situation: string;
}): string {
  return `You are a hospitality-training designer. Given a difficult-guest scenario, infer the hidden emotional layer that makes it realistic to practice: the thing a real guest feels but rarely says out loud. Write in natural, plain spoken English. Do not use em dashes; use commas or periods. Sound like a real person, not marketing copy.
